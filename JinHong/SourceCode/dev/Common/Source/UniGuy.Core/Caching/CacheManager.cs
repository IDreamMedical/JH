...
{
    /// <summary>
    /// 缓存管理器(主要用于创建不便宜但耗费内存不多的对象;如果是创建便宜并且耗费内存不多,建议不用缓存;如果是创建便宜但是耗费内存较多,则可以考虑使用WeakCacheManager)
    /// </summary>
    public class CacheManager
    {
        ...
        /// 缓存容器,这是一个静态的二级字典
        /// </summary>
        private static Dictionary<Type, Dictionary<string, CacheItem>> cacheContainer;
        ...
        /// </summary>
        /// <returns></returns>
        private static Dictionary<Type, Dictionary<string, CacheItem>> EnsureContainer()
        {
            return cacheContainer ?? (cacheContainer = new Dictionary<Type, Dictionary<string, CacheItem>>());
        }
        /// <summary>
        /// 获得确保存在的类缓存容器
        /// </summary>
        ...
        /// <returns></returns>
        private static Dictionary<string, CacheItem> EnsureTypeContainer(Type type)
        {
            Dictionary<Type, Dictionary<string, CacheItem>> ensureContainer = EnsureContainer();
            if (!(ensureContainer.ContainsKey(type) && ensureContainer[type] != null))
                ensureContainer[type] = new Dictionary<string, CacheItem>();
        ...
        /// <returns></returns>
        private static Dictionary<string, CacheItem> EnsureTypeContainer<T>()
        {
        ...
        public static void Put(string key, object cacheObject, TimeSpan duration)
        {
            ...
            EnsureTypeContainer(type)[key] = new CacheItem(cacheObject, duration);
        }
        /// <summary>
        ...
        public static void Put<T>(string key, T cacheObject, TimeSpan duration)
        {
            ...
            EnsureTypeContainer<T>()[key] = new CacheItem(cacheObject, duration);
        }
        /// <summary>
