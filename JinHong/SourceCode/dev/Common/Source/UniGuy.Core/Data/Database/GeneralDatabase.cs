...
{
    /// <summary>
    /// 一个通用的数据库访问辅助类(需要设置DatabaseObjectFactory)
    /// </summary>
    public class GeneralDatabase : IDatabase
    {
        /// <summary>
        /// 测试连接
        /// </summary>
        ...
        /// <returns></returns>
        public bool Test(IDbConnection conn)
        {
            if (conn == null)
                return false;
        ...
        #region Fields
        private IDatabaseObjectFactory _databaseFactory;
#if !CONCURRENT
        /// <summary>
        /// Sql连接列表
        /// </summary>
        private Dictionary<string, DbConnection> conns;
        ...
        /// 获得或者设置Sql连接列表
        /// </summary>
        public Dictionary<string, DbConnection> Conns
        {
            get { return conns; }
            set { conns = value; }
        }
        ...
        /// 构造器
        /// </summary>
        public GeneralDatabase(IDatabaseObjectFactory databaseFactory)
        {
        ...
        public GeneralDatabase(string providerName)
        {
            switch (providerName)
            {
                case "System.Data.SqlClient":
                    this._databaseFactory = new SqlObjectFactory();
                    break;
                //case "System.Data.OracleClient":
                //    this._databaseFactory = new OracleObjectFactory();
                //    break;
                case "System.Data.OleDb":
                    this._databaseFactory = new OleDbObjectFactory();
                    break;
                    //  默认使用SqlObjectFactory;
                default:
                    this._databaseFactory = new SqlObjectFactory();
                    break;
        ...
        private IDbConnection GetConnection(string connectionString)
        {
            try
            {
                return _databaseFactory.CreateConnection(connectionString);
            }
            catch (DbException ex)
            {
                OnDatabaseException(ex);
        ...
        private IDbConnection GetConnection(string connectionString)
        {
            ...
            {
                if (conns == null)
                    conns = new Dictionary<string, IDbConnection>();
                ...
                else
                {
                    try
                    {
                        conns.Add(connectionString, connection = _databaseFactory.CreateConnection(connectionString));
                    }
                    catch (DbException ex)
                    {
                        OnDatabaseException(ex);
        ...
        /// <returns></returns>
        /// Depricated
        public IDbCommand CreateCommand2(string connectionString, string sql, SqlParameterDictionary parameters)
        {
            IDbCommand comm = _databaseFactory.CreateCommand();
            comm.Connection = GetConnection(connectionString);
            comm.CommandText = sql;
            if (parameters != null)
                foreach (string key in parameters.Keys)
                    comm.Parameters.Add(_databaseFactory.CreateParameter(key, parameters[key] ?? DBNull.Value));
            return comm;
        }

        private void SqlizeCommand(IDbCommand comm, string sql, SqlParameterDictionary parameters)
        {
        ...
        /// <param name="parameters"></param>
        /// <returns></returns>
        public IDbCommand CreateCommand(string connectionString, string sql, SqlParameterDictionary parameters)
        {
            IDbCommand comm = _databaseFactory.CreateCommand();
            comm.Connection = GetConnection(connectionString);
        ...
        public IDbCommand CreateCommand(IDbTransaction tran, string sql, SqlParameterDictionary parameters)
        {
            IDbCommand comm = _databaseFactory.CreateCommand();
        ...
        public IDataReader Read(string connectionString, string sql, SqlParameterDictionary parameters, out string message)
        {
            ...
            using (IDbCommand comm = CreateCommand(connectionString, sql, parameters))
            {
#if !CONCURRENT
                lock (comm.Connection) // 避免共享资源commConnection冲突
                {
        ...
        public DataSet Select(string connectionString, string sql, SqlParameterDictionary parameters, out string message)
        {
            ...
            using (IDbCommand comm = CreateCommand(connectionString, sql, parameters))
            {
                ...
                try
                {
                    ...
                    _databaseFactory.CreateDataAdaptor(comm).Fill(resultDataSet);
        ...
        public int NonQuery(IDbTransaction tran, string sql, SqlParameterDictionary parameters, out string message)
        {
            ...
            using (IDbCommand comm = CreateCommand(tran, sql, parameters))
            {
#if !CONCURRENT
                lock (comm.Connection) // 避免共享资源commConnection冲突
                {
        ...
        public void OnDatabaseException(DbException ex)
        {
        ...
        public void OnDatabaseException(DbException ex, IDbCommand command)
        {
