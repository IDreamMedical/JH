...
namespace UniGuy.Core.Algorithms
{
    ...
    public static class CombinatoricsUtilities
    {
        ...
        /// </remarks>
        public static void GetCombinations<T>(IList<T> list, Action<IList<T>> action, int? resultSize, bool withRepetition)
        {
            if (list == null)
                throw new ArgumentNullException("list");
            if (action == null)
        ...
        public static IList<T> Permute<T>(IList<T> list, IList<int> indices)
        {
            if (list == null)
                throw new ArgumentNullException("list");
            if (indices == null)
