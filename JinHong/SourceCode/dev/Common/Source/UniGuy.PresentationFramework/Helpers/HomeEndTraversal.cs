...
{
    /// <summary>
    /// Home则到第一个可获得焦点的控件处, End则到最后一个(由于是Preview, 可以用在父元素上 )
    /// </summary>
    public class HomeEndTraversal
    {
        ...
        public static readonly DependencyProperty IsEnabledProperty =
            DependencyProperty.RegisterAttached("IsEnabled", typeof(bool), typeof(HomeEndTraversal), new UIPropertyMetadata(false, IsEnabledChanged));

        /// <summary>
        /// 必须的修饰键(除Shift键外, 因为Shift键已经限定用来控制方向, 默认为ModifierKeys.Control)
        /// </summary>
        public static readonly DependencyProperty ModifierKeysProperty =
            DependencyProperty.RegisterAttached("ModifierKeys", typeof(ModifierKeys), typeof(HomeEndTraversal), new PropertyMetadata(ModifierKeys.Control));
        ...
        static void Target_PreviewKeyDown(object sender, System.Windows.Input.KeyEventArgs e)
        {
            ...
            if ((mk & Keyboard.Modifiers) == mk)
            {
                if (e.Key == Key.Home)
                {
                    MoveFocus(e.OriginalSource, new TraversalRequest(Keyboard.IsKeyDown(Key.LeftShift) || Keyboard.IsKeyDown(Key.RightShift) ? FocusNavigationDirection.Last : FocusNavigationDirection.First));
                    e.Handled = true;
                }
