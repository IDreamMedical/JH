...
using se = System.Data.SqlClient.SqlException;

namespace UniGuy.Core.Data
{
    /// <summary>
    /// 基于System.Data.SqlClient的数据库访问辅助类
    /// </summary>
    public class SqlDatabase : IDatabase
    {
        ...
        /// <returns></returns>
        public bool Test(SqlConnection conn)
        {
            if (conn == null)
                return false;
        ...
        /// </summary>
#if !CONCURRENT
        private Dictionary<string, SqlConnection> conns;
        ...
        /// 获得或者设置Sql连接列表
        /// </summary>
        public Dictionary<string, SqlConnection> Conns
        {
            get { return conns; }
            set { conns = value; }
        }
        ...
        private SqlConnection GetConnection(string connectionString)
        {
            ...
            {
                if (conns == null)
                    conns = new Dictionary<string, SqlConnection>();
                ...
                else
                {
                    try
                    {
                        conns.Add(connectionString, connection = new SqlConnection(connectionString));
                    }
                    catch (se ex)
                    {
                        OnDatabaseException(ex);
        ...
        /// <returns></returns>
        /// Depricated
        public SqlCommand CreateCommand2(string connectionString, string sql, SqlParameterDictionary parameters)
        {
        ...
        private void SqlizeCommand(SqlCommand comm, string sql, SqlParameterDictionary parameters)
        {
        ...
        /// <param name="parameters"></param>
        /// <returns></returns>
        public SqlCommand CreateCommand(string connectionString, string sql, SqlParameterDictionary parameters)
        {
            SqlCommand comm = new SqlCommand();
            comm.Connection = GetConnection(connectionString);
        ...
        public SqlCommand CreateCommand(IDbTransaction tran, string sql, SqlParameterDictionary parameters)
        {
            SqlCommand comm = new SqlCommand();
            comm.Connection = (SqlConnection)tran.Connection;
            comm.Transaction = (SqlTransaction)tran;
        ...
        public IDataReader Read(string connectionString, string sql, SqlParameterDictionary parameters, out string message)
        {
            ...
            using (SqlCommand comm = CreateCommand(connectionString, sql, parameters))
            {
#if !CONCURRENT
                lock (comm.Connection) // 避免共享资源commConnection冲突
                {
            ...
            using (SqlCommand comm = CreateCommand(connectionString, sql, parameters))
            {
                ...
                lock (comm.Connection) // 避免共享资源commConnection冲突
                {
                    ...
                    try
                    {
                        ...
                        new SqlDataAdapter(comm).Fill(resultDataSet);
        ...
        public int NonQuery(IDbTransaction tran, string sql, SqlParameterDictionary parameters, out string message)
        {
            ...
            using (SqlCommand comm = CreateCommand(tran, sql, parameters))
            {
#if !CONCURRENT
                lock (comm.Connection) // 避免共享资源commConnection冲突
                {
        ...
        public void OnDatabaseException(se ex)
        {
        ...
        public void OnDatabaseException(se ex, SqlCommand command)
        {
