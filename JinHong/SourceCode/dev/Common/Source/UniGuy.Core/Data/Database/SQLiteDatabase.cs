...
namespace UniGuy.Core.Data
{
    ...
    public class SQLiteDatabase : IDatabase
    {
        ...
        /// <returns></returns>
        public bool Test(SQLiteConnection conn)
        {
            if (conn == null)
                return false;
        ...
        /// </summary>
#if !CONCURRENT
        private Dictionary<string, SQLiteConnection> conns;
        ...
        /// 获得或者设置Sql连接列表
        /// </summary>
        public Dictionary<string, SQLiteConnection> Conns
        {
            get { return conns; }
            set { conns = value; }
        }
        ...
        private SQLiteConnection GetConnection(string connectionString)
        {
            ...
            {
                if (conns == null)
                    conns = new Dictionary<string, SQLiteConnection>();
                ...
                else
                {
                    try
                    {
                        conns.Add(connectionString, connection = new SQLiteConnection(connectionString));
                    }
                    catch (SQLiteException ex)
                    {
                        OnDatabaseException(ex);
        ...
        /// <returns></returns>
        /// Depricated
        public SQLiteCommand CreateCommand2(string connectionString, string sql, SqlParameterDictionary parameters)
        {
            ...
            if (parameters != null)
                foreach (string key in parameters.Keys)
                    comm.Parameters.Add(new SQLiteParameter(key, parameters[key] ?? DBNull.Value));
            return comm;
        }

        private void SqlizeCommand(SQLiteCommand comm, string sql, SqlParameterDictionary parameters)
        {
        ...
        odc0:
            ...
            if (s >= 0)
            {
                ...
                if (e >= 0)
                {
                    ...
                    //  TODO...20141008 这里好像有BUG, 相同key的没法加入第二次
                    comm.Parameters.Add(new SQLiteParameter(comm.Parameters.Contains(key) ? Guid.NewGuid().ToString() : key, parameters[key] ?? DBNull.Value));
        ...
        /// <param name="parameters"></param>
        /// <returns></returns>
        public SQLiteCommand CreateCommand(string connectionString, string sql, SqlParameterDictionary parameters)
        {
            SQLiteCommand comm = new SQLiteCommand();
            comm.Connection = GetConnection(connectionString);
        ...
        public SQLiteCommand CreateCommand(IDbTransaction tran, string sql, SqlParameterDictionary parameters)
        {
            SQLiteCommand comm = new SQLiteCommand();
            comm.Connection = (SQLiteConnection)tran.Connection;
            comm.Transaction = (SQLiteTransaction)tran;
        ...
        {
            message = null;
            SQLiteConnection conn = GetConnection(connectionString);
#if !CONCURRENT
        ...
        public IDataReader Read(string connectionString, string sql, SqlParameterDictionary parameters, out string message)
        {
            ...
            using (SQLiteCommand comm = CreateCommand(connectionString, sql, parameters))
            {
#if !CONCURRENT
                lock (comm.Connection) // 避免共享资源commConnection冲突
                {
            ...
            using (SQLiteCommand comm = CreateCommand(connectionString, sql, parameters))
            {
                ...
                lock (comm.Connection) // 避免共享资源commConnection冲突
                {
                    ...
                    try
                    {
                        ...
                        new SQLiteDataAdapter(comm).Fill(resultDataSet);
        ...
        public int NonQuery(IDbTransaction tran, string sql, SqlParameterDictionary parameters, out string message)
        {
            ...
            using (SQLiteCommand comm = CreateCommand(tran, sql, parameters))
            {
#if !CONCURRENT
                lock (comm.Connection) // 避免共享资源commConnection冲突
                {
        ...
        public void OnDatabaseException(SQLiteException ex)
        {
        ...
        public void OnDatabaseException(SQLiteException ex, SQLiteCommand command)
        {
