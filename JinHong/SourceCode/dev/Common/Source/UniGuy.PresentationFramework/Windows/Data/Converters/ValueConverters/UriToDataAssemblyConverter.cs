...
{
    /// <summary>
    /// 通过Uri指定的内容反序列化为DataAssembly
    /// </summary>
    [Author("UniGuy"), LastModifyTime("2010-11-27")]
    [ValueConversion(typeof(Uri), typeof(DataAssembly))]
    public class UriToDataAssemblyConverter : MarkupExtension, IValueConverter
    {
        #region Fields
        ...
        #region Ctor
        static UriToDataAssemblyConverter()
        {
            cache = new Dictionary<string, WeakReference>();
        }
        public UriToDataAssemblyConverter() { }
        #endregion
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (source != null)
            {
                ...
                XmlSerializer xs = new XmlSerializer(typeof(DataAssembly));
                DataAssembly da = (DataAssembly)xs.Deserialize(stream);
                if (usingCache)
                    cache[key] = new WeakReference(da);
                return da;
            }
