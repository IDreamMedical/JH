...
namespace UniGuy.Core.Data
{
    ...
    /// Sql数据库表调整器
    /// </summary>
    public class SqlDatabaseTableAdjuster : DatabaseTableAdjusterBase
    {
        #region Fields
        /// <summary>
        /// 数据库
        /// </summary>
        /// <remarks>在UniGuy.Sql中可以实现另外一套, 不过这里应该是SqlSqlProvider. TODO</remarks>
        ...
        #region Constructors
        public SqlDatabaseTableAdjuster() { }
        public SqlDatabaseTableAdjuster(IDatabase database) { this._database = database; }
        public SqlDatabaseTableAdjuster(IDatabase database, string connectionString) : this(database) { _connectionString = connectionString; }
        #endregion
        ...
        {
            string sql = "SELECT name, length, isnullable FROM syscolumns WHERE id=(SELECT id FROM sysobjects WHERE Name={TableName})";
