...
namespace UniGuy.Controls.Converters
{
	...
	public class MultiConverterGroup : DependencyObject, IMultiValueConverter
	{
		...
		public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
		{
			...
			if(Steps[Steps.Count - 1].Converters.Count != 1)
                throw new Exception("FinalStepMustHaveOneConverter");

			foreach (MultiConverterGroupStep step in Steps)
			{
                if (step.Converters.Count == 0)
                    throw new Exception("EachStepMustHaveAtLeastOneConverter");
		...
		public object[] ConvertBack(object value, Type[] targetTypes, object parameter, CultureInfo culture)
		{
			...
			for (int i = Steps.Count - 1; i >= 0; --i)
			{
				...
				if(step.Converters.Count != stepValues.Length)
                    throw new Exception("NumberOfConvertersInStepMustEqualNumberOfValuesFromPreviousStep");
